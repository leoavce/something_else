...
btnUp.addEventListener("click", () => run(btnUp, async () => {
  const em = email.value.trim(); const pw = pass.value;
...
btnIn.addEventListener("click", () => run(btnIn, async () => {
  const em = email.value.trim(); const pw = pass.value;
...
// Enter로 로그인
form.addEventListener("submit", (e) => { e.preventDefault(); btnIn.click(); });
